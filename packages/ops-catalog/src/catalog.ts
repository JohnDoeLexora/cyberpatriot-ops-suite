...
export const catalog: readonly OpDefinition[] = Object.freeze([
  ...
  op(
    ...
    "windows",
    "mutate",
    "Close PrintNightmare-class remote driver install: RestrictDriverInstallationToAdministrators, PointAndPrint no-warning elevation off, RegisterSpoolerRemoteRpcEndPoint disabled, RPC auth privacy on. Does not stop the local spooler unless the README says printing is unused. Live requires confirm:true." +
      KOSHER,
    "Simulates PointAndPrint restrict + remote RPC endpoint disabled; Spooler left running for local print",
    mutateFlagParams,
  ),
  op(
    ...
    "RunAsPPL=0, RunAsPPLBoot unset — LSA not protected",
  ),
  op(
    ...
    "Credential Guard not running; ConfigurableTCB off; no secret material in the result",
  ),
  op(
    "audit-secure-boot",
    ...
    "SecureBoot=false, SetupMode=true on the demo fixture",
  ),
  op(
    "audit-wifi-profiles",
    ...
    "Open SSID 'CP-GUEST'; leftover WPA2 'HomeRouter' (key omitted); one enterprise profile ok",
  ),
  op(
    ...
    "Simulates ScriptBlockLogging+ModuleLogging+Transcription on; LanguageMode FullLanguage unless constrainedLanguage",
    powershellHardenParams,
  ),
  op(
    ...
    "Simulates Set-SmbClientConfiguration EnableSMB1Protocol=false and mrxsmb10 disabled",
    mutateFlagParams,
  ),
  op(
    "audit-dns-client",
    ...
    "DNS 10.13.37.1 (unexpected); DoH unset; NRPT empty; hosts poisoning is a separate op",
  ),
  op(
    "audit-windows-roles",
    ...
    "AD-Domain-Services Installed (unexpected); DNS Installed; DHCP Installed; IIS present",
  ),
  op(
    "harden-null-session",
    ...
    "Simulates the four LSA restrict values set to the hardened baseline",
    mutateFlagParams,
  ),
  op(
    ...
    "Simulates blacklisting dccp/sctp/cramfs/hfs; usb-storage left loaded unless usbStorage=true",
    kernelModuleParams,
  ),
  op(
    ...
    "Simulates aa-enforce apache2 mysqld ntpd ping; 3 complain profiles remain unnamed",
    mutateFlagParams,
  ),
  op(
    ...
    "Simulates writing 20auto-upgrades Unattended-Upgrade 1 and enabling the package",
    mutateFlagParams,
  ),
  op(
    "audit-mail-services",
    ...
    "postfix inet_interfaces=all, mynetworks includes 0.0.0.0/0, disable_vrfy_command=no",
  ),
  op(
    "audit-database-bind",
    ...
    "mysqld bind-address=0.0.0.0; skip-grant-tables in systemd override; pg_hba host all all 0.0.0.0/0 trust",
  ),
  op(
    "audit-php-hardening",
    ...
    "expose_php=On, allow_url_include=On, disable_functions empty; /var/www/html/info.php present",
  ),
  op(
    "audit-snap-flatpak",
    ...
    "snap: steam, discord; flatpak: org.videolan.VLC, com.anydesk.Anydesk; core snaps ignored",
  ),
  op(
    ...
    "Simulates systemctl mask ctrl-alt-del.target and disable serial-getty@ttyS0",
    mutateFlagParams,
  ),
  op(
    "audit-ipv6-privacy",
    ...
    "use_tempaddr=0, accept_ra=1, forwarding=1; disableIPv6 not applied in the demo",
    ipv6Params,
  ),
  op(
    ...
    "journald Storage=volatile; /var/log/journal missing; rsyslog inactive",
  ),
  op(
    ...
    "Firefox homepage http://10.13.37.1/pwn; system proxy 10.13.37.1:8080; one unpacked Chrome extension id",
  ),
  op(
    "harden-usb-storage",
    ...
    "Simulates autorun off + Deny_Execute on removable; USBSTOR left enabled unless disableUsbStorage",
    usbStorageParams,
  ),
  op(
    "audit-time-timezone",
    ...
    "timesyncd inactive; NTP 10.13.37.1; timezone Etc/GMT+12 (suspicious); RTC not NTP-synced",
  ),
  op(
    "export-coach-packet",
    ...
    "both",
    "read",
    "Assemble a redacted handoff ZIP for a coach: SUMMARY.md, findings.json, user/service/port inventories (no hashes), checklist snapshot. Never includes shadow/SAM, private keys, Wi-Fi PSKs, cookies, or CCS URLs. Distinct from export-evidence-bundle: this is the coach-facing packet, still on-image only." +
      KOSHER,
    "ZIP listing SUMMARY.md + findings.json + inventories; redacted=true; ccsContacted=false; no secrets",
    coachPacketParams,
  ),
]);
